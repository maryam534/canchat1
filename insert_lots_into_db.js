...
async function withRetries(fn, tries = 3, baseDelayMs = 1200) {
    ...
    for (let i = 0; i < tries; i++) {
        try {
            ...
            const msg = (e.response && e.response.data && e.response.data.error && e.response.data.error.message) ? e.response.data.error.message : e.message;
...
async function getEmbedding(text) {
    ...
    const emb = (res && res.data && res.data.data && Array.isArray(res.data.data) && res.data.data[0]) ? res.data.data[0].embedding : undefined;
...
async function upsertSaleTx(client, jsonData, firmPk) {
    ...
    const {
        startDate,
        endDate
    } = parseEventDate(jsonData.eventdate || '');
    const salelogo = (jsonData.saleInfo && jsonData.saleInfo.saleLogo) ? jsonData.saleInfo.saleLogo : '';
    const salesource = (jsonData.contact && jsonData.contact.website) ? jsonData.contact.website : '';
  ...
  `, [firmPk, saleNo, salename, startDate, endDate, salelogo, salesource]);
    ...
    return {
        salePk: r.rows[0].sale_pk,
        saleEndDate: endDate || null
    };
}
...
async function upsertLotTx(client, lot, jsonData, firmPk, salePk) {
    ...
    const {
        price: opening,
        currency
    } = extractPriceAndCurrency(lot.startingprice);
    const {
        price: realized
    } = extractPriceAndCurrency(lot.realizedprice);
...
async function insertSingleLot(lot, eventData) {
    ...
    try {
        ...
        const {
            salePk
        } = await upsertSaleTx(client, eventData, firmPk);
    ...
    try {
        ...
        for (const file of jsonFiles) {
            ...
            try {
                ...
                const {
                    salePk
                } = await upsertSaleTx(client, jsonData, firmPk);
            ...
            try {
                ...
                const toEmbed = lotPkMap.filter(({
                    lotPk
                }) => !alreadySet.has(lotPk));
